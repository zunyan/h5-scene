(function() {
	...
	$.fn.render = function(pages) {
		...
		$(this).on("mousedown touchstart", function(ev) {
			var me = $(this),
				ox = ev.pageX ||ev.changedTouches[0].pageX,
				oy = ev.pageY ||ev.changedTouches[0].pageY;
			stage.removeClass("ani");
			$(this).on("mousemove touchmove", function(ev) {
				var nx = ev.pageX||ev.changedTouches[0].pageX,
					ny = ev.pageY||ev.changedTouches[0].pageY;
				stage.css("transform", "translateY(" + ((ny - oy)/2 + offset) + "px)")
	 ...
	 * @author kezy
	 */
	$.fn.cssAnimate = function(animations) {
		var oldAni = "",
