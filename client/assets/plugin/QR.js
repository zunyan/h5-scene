(function(window, scope, element, attrs, ctrl) {
	window.qrcode = window.qrcode || function(h, element) {
		var s;
		...
		for (var j = tempobj, l = tempObj2, m = 0; 8 > m; m++)
			l.EXP_TABLE[m] = 1 << m;
		for (m = 8; 256 > m; m++)
		...
		h = defSet;
		var qrcode = new o(h.typeNumber, h.correctLevel);
		qrcode.addData(h.text);
		qrcode.make();
		var c = null;
		if ("canvas" == h.render) {
			...
			c.width = h.width;
			c.height = h.height;
			for (var d = c.getContext("2d"), b = h.width / qrcode.getModuleCount(), e = h.height / qrcode.getModuleCount(), f = 0; f < qrcode.getModuleCount(); f++)
			...
			d = h.width / qrcode.getModuleCount();
			b = h.height / qrcode.getModuleCount();

			for (e = 0; e < qrcode.getModuleCount(); e++) {
				f = $("<tr></tr>").css("height", b + "px");
				c.append(f);
				for (i = 0; i < qrcode.getModuleCount(); i++) {
					var td = $("<td></td>").css({
						"width": d + "px",
						"background-color": qrcode.isDark(e, i) ? h.foreground : h.background,
					});
					f.append(td);
				}
	...
	var set = {
		text: scope.$eval(attrs.qrcode),
		width: attrs.width || 256,
		height: attrs.width || 256,
		render: attrs.render || (isSupportCanvas() ? 'canvas' : 'table')
	};

	var res = null ;

	if (attrs.config)
		set = angular.extend(set, scope.$eval(attrs.config));

	if (set.text) {
		res = qrcode(set, element);
		element.append(res);
	}

	if(attrs.async){
		scope.$watch(attrs.qrcode,function(newValue, oldValue){
...
})(window, scope, element, attrs, ctrl);